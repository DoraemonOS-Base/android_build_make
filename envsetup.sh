...
EOF
    ...
    __print_doraemon_functions_help
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^doraemon_") ; then
        DORAEMON_BUILD=$(echo -n $1 | sed -e 's/^doraemon_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $DORAEMON_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        DORAEMON_BUILD=
    fi
    export DORAEMON_BUILD
